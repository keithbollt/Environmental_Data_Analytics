...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()  # this is a different theme
print(O3plot2)

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()  # so is this 
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()  # Kateri likes this one the best
print(O3plot4)
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) +    # name a theme, pick a theme as the base, then make some modifications for fun
...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme    # this sets the theme for the plot
print(O3plot5)  

theme_set(mytheme)  # this sets the theme at the start
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 0, ymax = 50, fill = "green") +  # geom_rect creates the yellow and green zones
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 50, ymax = 100, fill = "yellow") +
  geom_point() +
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +
  geom_text(x = as.Date("2019-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + # affects the x axis
  scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), # affects the x axis, formats it as a date
    date_breaks = "2 months", date_labels = "%b %y") +
 ylab(expression("O"[3]* " AQI Value")) +  # affects the y axis 
  theme(axis.text.x = element_text(angle = 45,  hjust = 1))  # affects the axis text
print(O3plot7)  
...
```{r, warning = FALSE}     
#install.packages("viridis")   # this installs color palatte packages 
...
NvsP2 <-
  ...
  xlab(expression("TP (micrograms/L)")) +  # lets you re-label the axes
  ylab(expression("TN (micrograms/L)")) +
  # change your legend labels here
  labs(shape = "Lake")  # changes the legend label
  scale_shape_manual(values = c(15, 17)) +
  scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1) +  # this is how you would change the color of the scale bar
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(x = "Lake Name", y = "Concentration in micrograms/L")
  geom_text()
# place your additional edits here
  scale_y_continuous(expand = c(0, 0)) +
  #scale_color_brewer(palette = "YlGnBu") +   # this changes the colors
