...
```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) # predicts irradiance by depth
# another, more efficent way to format the lm function
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression) # tells you the intercept, slope, r^2 value

# Correlation: gives you the R value
...
> ANSWER: give the r^2 value, p value, degrees of freedom, equation, and say what model you ran
...
plot(irradiance.regression) # lets you look at a general fit
`` 
...
# this relationship looks exponential, so a linear fit isn't good
```
...
```{r, fig.height = 3, fig.width = 4}
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, irradianceWater != 0) # remove 0 values bc you can't take the log of 0
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) # compares log of irradiance to depth
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  scale_y_log10() +  # plots y on a log scale
  geom_point(alpha = 0.5) + # changes transparency
...
TPplot <- ggplot(PeterPaul.chem.nutrients, 
                 aes(x = dissolvedOxygen, y = tp_ug, color = depth)) +  # coloring by one of the variables lets you graph 3 variables in 2D
  geom_point() +
  xlim(0, 20)
print(TPplot)
```
...
library(corrplot)  # lets you look at lots of correlations in a dataset at once
PeterPaulnutrients <- 
  PeterPaul.chem.nutrients %>%  # note that this is a pipe function
  select(tn_ug:po4) %>%
  na.omit()  # omit the na's
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse")  # creates a correlation plot
...
```{r}
PeterPaul.naomit <- na.omit(PeterPaul.chem.nutrients)  # gets rid of the na's
TPAIC <- lm(data = PeterPaul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)
step(TPAIC)  # the lower the aic, the better the correlation
TPmodel <- lm(data = PeterPaul.naomit, tp_ug ~ temperature_C + tn_ug)
summary(TPmodel)  # run the best fit model by itself once you figure out which one is best
...
```{r, fig.height = 3, fig.width = 4}
# main effects
TNancova.main <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename + depth) # gives you different regressions for the different variables
summary(TNancova.main)  # the first intercept number is for paul lake, the second is for peter lake
