...
- code output and text in one document, just code in another
- convert code to a useful format (pdf, etc.)
-And many more...
...
Dataset    |   Description
-------------------  |   ------------------------------------------------
EPA Air    |   This dataset has data on air quality, specifically on ozone and something else I forget
Hydrology  |   This dataset has USGS hydrology data on sites that I don't remember the locations of and don't feel like looking for
NTLR Data  |   This dataset looks at chemistry and physics, and also nutrient data from several lakes in Wisconsin, including Peter and Paul Lake, but there are something like nine lakes in total. From every indication, although we cannot be 100 percent sure without visiting the lakes themselves, these lakes are filled with water, and not, as some are led to believe, melted cheese or Milwaukee-brewed beer.
ECOTOX Neonicotoids Data | This dataset contains data on dead insects and the chemicals that killed them, I think
...
library(tidyverse)
library(knitr)
mytheme <- theme_set(theme_classic(base_size = 13.14159))
lakedata <- read.csv("V:/ENV_872_Project_Directory/Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
head(lakedata)
...
```{r}
processed_NTLR <- lakedata %>%
  filter(-lakeid, - depth_id, - comments) %>%
  filter(depth == 0)
```
