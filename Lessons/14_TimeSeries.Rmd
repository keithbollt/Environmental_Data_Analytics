...
pettitt.test(Peter.nutrients.surface$tp_ug) # the p-value is small, so there is a change point.The test only tells you a relative change point (row#)

# Run separate Mann-Kendall for each change point
mk.test(Peter.nutrients.surface$tp_ug[1:34])  # no trend
mk.test(Peter.nutrients.surface$tp_ug[35:132])  # not a significant trend

# Is there a second change point?
pettitt.test(Peter.nutrients.surface$tp_ug[35:132]) #35+79=114
...
pettitt.test(Paul.nutrients.surface$tp_ug)  #the reference lake doesn't have a trend
