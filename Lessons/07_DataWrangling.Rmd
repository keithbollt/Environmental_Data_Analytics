...
getwd()
library(tidyverse)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv") #loading the data into environments
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #shows where the depth is equal to 0, you need the comma because R cares about rows and columns

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) #filtering depths with dplyr

# Did the methods arrive at the same result?
head(NTL.phys.data.surface1) #matrix filtering keeps the rows' original numbers
dim(NTL.phys.data.surface1)
head(NTL.phys.data.surface2) #stores rows as new row names 1,2,3,4,5,6,7,etc
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #looking just at these two lakes' data
...
#%in% means include; this generates the same dataset as line 92. Don't forget the c
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305)  #, means 'and' in dplyr
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #all 4 of these lines do the same thing
...
NTL.phys.data.Tuesdaythe90s <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 >=1990, year4<= 1999)


```
Question: Why don't we filter using row numbers?
<<<<<<< HEAD
> ANSWER: rows change if you filter them. also, rows don't have much information in their title, and you would need to check what rows you want
=======

> ANSWER: 
>>>>>>> 9e51cd169427ab7623c3dbe57fa52f82c357f150
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #sorts data by depths
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) # sorts data by depths in descending order
...
look at the dataset you just generated to answer this question
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) # lets you select different columns (filter selects rows)
#selects lake name and everything between sample data and temperature
...
NTL.phys.data.temps <- mutate(NTL.phys.data, temperature_F = (temperature_C*9/5) + 32) #converting temperature to F
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%        # you are staying within the same line of code, so you don't need to repeat the dataset name
...
```{r}
write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv") #saving a dataset from Environments into a file
# ./ moves the file down into 'processed'
```
