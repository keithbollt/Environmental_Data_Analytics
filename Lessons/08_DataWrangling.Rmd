...
#filtering out only peter and paul lake data
...
write.csv(NTL.nutrient.data.PeterPaul, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaul_Processed.csv") # saving our dataframe as a csv file
#rownames=false makes sure that we do not save the first column, which is basically useless in this case
...
#if you wanted to remove these columns instead of keep them, put a - in front of the name
...
#creates nutrient and concentration columns, then gathers the columns and rows of the types and concentrations of the nutrients
...
#spreads the gathered data back into separate columns
...
#separates date into year, month, and day columns; gets rid of original sampledate column
```
...
NTL.phys.nutrient.data.PeterPaul <- mutate(NTL.phys.nutrient.data.PeterPaul, month = month(sampledate)) #lets you add a column called month and fills it out with the month of sampledate column

# reorder columns to put month with the rest of the date variables
NTL.phys.nutrient.data.PeterPaul <- select(NTL.phys.nutrient.data.PeterPaul, lakename, year4, month, sampledate:po4) #moves month column next to year
...
#first, last function is a more reproducible way of finding an interval
```
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
# filters out rows that have no na's in all three columns
#will produce separate means, std deviations for each lake because the lakes are grouped
