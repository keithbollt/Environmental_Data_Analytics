...
#kind of looks like North Carolina
```
...
counties_sf<- st_read('./Data/Spatial/cb_2017_us_county_20m.shp') %>% 
  filter(STATEFP == 37) #Filter for just NC Counties, turns a spatial dataset into an R dataframe
...
state_sf <- counties_sf %>% 
  summarize(ALAND = sum(ALAND))  #this aggregates all the counties in NC into a single shape

#View the data
mapview(state_sf) #This maps the state of North Carolina
...
#Right order, the order of the data layers is important
ggplot() +
  geom_sf(data = state_sf_utm, color='red',size=2) +
  geom_sf(data = counties_sf_utm, aes(fill = ALAND), color = 'white')  +
...
huc0302 <- huc2_utm %>%
  filter(SUB == "0302") 
mapView(huc0302)
huc0302_sub <- st_intersection(huc0302,counties_sf_utm)
  
#Map the product, showing values by land area
mapview(huc0302_sub['ALAND'])
```
...
#for the HW, you have to use st_as_sf to convert to a spatial data frame
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>% 
  st_convex_hull()  #this creates a polygon connecting the four centroids
...
triCo_centroids_voronoi <- triCo_centroids %>% 
  st_union() %>% 
  st_voronoi()  #shows what area is closest to a given point
...
selCounties <- counties_sf_utm[resultMask,]  #only one county is true
...
#buffer the coordinate by 30km at the beginning
