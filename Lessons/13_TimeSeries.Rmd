...
The coefficients of a hierarchical model are divided into two categories: **fixed effects** and **random effects.** A **fixed effect** is a factor whose levels are experimentally determined or whose interest lies in the effects of each level (e.g., covariates, treatments, interactions). A **random effect** is a factor whose levels are sampled from a larger population, or whose interest lies in the variation among them rather than the specific effect of each level. In choosing whether you are dealing with a fixed or a random effect, consider the following questions: 
  ...
  + Do you have a particular interest in the studied factor level? Yes-> fixed

  + Have you included all possible levels in the study?  Yes-> fixed
  
  + Do you have interest in the variance among levels?   Yes-> random
  
  + Do you have interest in generalizing to factor levels that you did not study? Yes-> random
...
TempTest.auto <- lme(data = PeterPaul.summertemp, #specify the dataset
...
ACF(TempTest.auto)
# we care about the lag of 1's value. This tells us how much temperature is autocorrelated within a given year (it's 42%)
```
...
TempTest.mixed <- lme(data = PeterPaul.summertemp,
                     ...
                     correlation = corAR1(form = ~ sampledate/lakename|Week, value = 0.423), #correlation from previous model, sampledate/lakename because the model can only do one lake at a time
                     #define method as restricted maximum likelihood
                     method = "REML")
summary(TempTest.mixed) #incercept is for the first date and the first lake
...
#the model that incorporates random effects is a better model
...
# generated a sample date based off of the intercept and slope from our models
```
