...
#head(USGS.flow.data) #shows the first six lines
...
USGS.flow.data$datetime <- as.character(USGS.flow.data$datetime, format = "%m/%d/%y") 
class(USGS.flow.data$datetime)
```
...
create.early.dates <- (function(d) {
       ...
       })    # the paste function combines the two things inside the parentheses
#
...
> Answer: in social science, a NA may have meaning. You might want to take out the NAs, or leave them in in other circumstances

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #creates a dataframe with only non-NA rows
dim(USGS.flow.data)
dim(USGS.flow.data.complete) #much smaller
...
ggplot(USGS.flow.data.complete, aes(x = discharge.mean.approval)) + #generates graph of approved vs pending data
  geom_bar()
```
...
#
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10) #changing the width of the bars to 10
...
ggplot(USGS.flow.data.complete, aes(x = discharge.mean)) +
  ...
  scale_x_continuous(limits = c(0, 500)) #setting the x axis to be from 0 to 500 in order to zoom in on the majority of data
...
ggplot(USGS.flow.data.complete) +
  ...
  scale_x_continuous(limits = c(0, 10)) +
  theme(legend.position = "top") # this graph colors within the aesthetics as to whether the data is approved or pending
...
#the wider the violin, the more data points at this gage height
```
