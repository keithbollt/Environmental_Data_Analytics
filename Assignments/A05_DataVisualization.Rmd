...
```{r}
#4
phosphorus_vs_phosphate <- ggplot(lake.chem.physics.tidy, aes(x = po4, y = tp_ug, color = depth, shape = lakename))+
  geom_point()+
  xlab(expression ("Phosphate (μg/L)"))+
  ylab(expression ("Total Phosphorus (μg/L)"))+
  labs(shape = "Lake")+
...
```{r}
#5
nutrients_by_date2 <- ggplot(lake.chem.nutrients.gathered, aes(x= sampledate, y= concentration, color = depth))+
  geom_point()+
  ylab(expression("Concentration (μg/L)"))+
  xlab(expression("Date"))+
  facet_grid(nutrient ~ concentration)
print(nutrients_by_date2)
